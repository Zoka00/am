...
ControllerAll = false
local ListSudos ={Sudo_Id,1614314857,2002229504,5185547990}  
...
msg.The_Controller = 1
elseif tonumber(msg.sender.user_id) == 2002229504 then
...
local N = (Redis:get(Saidi.."Name:Bot") or "تربو")
...
return LuaTele.sendText(msg_chat_id,msg_id,'◍ ⋮  تم استرجاع المجموعات من نسخه تربو')
else
...
NamesBot = (Redis:get(Saidi.."Name:Bot") or "تربو")
...
return LuaTele.sendText(msg_chat_id,msg_id,'◍ ⋮  اليك قسم الالعاب من سورس تربو ',"md",false, false, false, false, reply_markup)
end
---زخرفة ----
...
local NamesBot = (Redis:get(Saidi.."Name:Bot") or "تربو")
local BotName = {
'• مرحبا انا '..NamesBot..'',
}
...
if text == (Redis:get(Saidi.."Name:Bot") or "تربو") then
local photo = LuaTele.getUserProfilePhotos(Saidi)
local Jabwa = LuaTele.getUser(Saidi)
local sudo_info = LuaTele.getUser(Sudo_Id)
local sudo_name = sudo_info.first_name
...
local NamesBot = (Redis:get(Saidi.."Saidi:Name:Bot") or "تربو")
local BotName = {
'نعم يروحي',
...
if text == (Redis:get(Saidi.."Name:Bot") or 'تربو').." غادر" or text == 'غادر' or text == 'بوت غادر' then
if not msg.Developers then
return LuaTele.sendText(msg_chat_id,msg_id,'\n*◍ ⋮  هاذا الامر يخص { '..Controller_Num(3)..' }* ',"md",true)  
end
